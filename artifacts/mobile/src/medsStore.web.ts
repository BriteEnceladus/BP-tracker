/**
 * Web medications store — per-record AES-256-GCM encryption (same format as native).
 */
...
async function decryptMed(
  ...
  try {
    ...
    if (item.name) return item as Medication;
  } catch (e) {
    console.warn('[medsStore.web] decrypt failed', e);
  }
  return null;
}
