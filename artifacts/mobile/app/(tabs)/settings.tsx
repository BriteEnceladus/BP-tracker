...
export default function SettingsScreen() {
  ...
  const exportCsv = async () => {
    try {
      const visibleBp = isPremium ? readings : getReadingsForDays(readings, FREE_HISTORY_DAYS);
      const visibleGlucose = isPremium ? glucose : getGlucoseReadingsForDays(glucose, FREE_HISTORY_DAYS);
      if (visibleBp.length === 0 && visibleGlucose.length === 0) {
        Alert.alert('No Data', `Free export includes the last ${FREE_HISTORY_DAYS} days. Older logs stay on this device and export with Pro.`);
        return;
      }
      await shareCsvFile(readingsToCsv(visibleBp), `bp_readings_${new Date().toISOString().split('T')[0]}.csv`);
